...
function AgendaWeekView(element, calendar) {
	...
	function render(date, delta) {
		...
		t.visEnd = visEnd;
                //alert("hola");
		renderAgenda(weekends ? 7 : 5, 1);//The second paremeter is an added midification for validate
                // wether the allDaySlot text has to be changed or not. This validation takes place on rendering.
	}
...
function AgendaView(element, calendar, viewName) {
	...
	function renderAgenda(c,labels) {
		...
		if (!head) { // first time rendering, build from scratch
			...
			for (i=0; i<colCnt; i++) {
				s += "<th id='"+d+"' class='fc-" +
					dayIDs[d.getDay()] + ' ' + // needs to be first
					tm + '-state-default agendaDay' +//agendaDay class and id parameter had been added, those are not be found wihtin the original plugin
					"'>" + formatDate(d, colFormat) + " h</th>";
				addDays(d, dis);
				if (nwe) {
					skipWeekend(d, dis);
				}
			...
			for (i=0; i<colCnt; i++) {
				s += "<td class='fc-" +
					...
					"'><div class='fc-day-content'><div id='"+d+"_div' style='height:33px; text-align:center; position:relative;line-height:28px'> Liberar "+formatDate(d,"dddd")+"</div></div></td>";/*This has customized in order to display the release option for the current timetable*/
				addDays(d, dis);
				if (nwe) {
					skipWeekend(d, dis);
				}
			...
			//alert(this.getView());
			// redo column header text and class
                        if(labels==undefined){//This statement has been added as a personal modification. It changes the allDaySlot text only within agendaDay view.
                            $("div[id*='_div']").each(function(i, div){
                                $(div).html("Liberar "+formatDate(d,"dddd"));
                            });
                        }
			head.find('tr:first th').slice(1, -1).each(function(i, th) {
				$(th).text(formatDate(d, colFormat));
                                
                                $(th).attr("id",d);//This change has been added as a personal modificaction. This control the change in the id each time we change to a previous or next week
